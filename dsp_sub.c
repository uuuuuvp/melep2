...
{// ifact 为插值因子	
	...
	if (ifact == 0)
		v_equ(out, prev, size);		// 为 0 则不插值， 输出为没插值的
	else if (ifact == ONE_Q15)		// one_Q15 = 32767, 很奇怪，为啥是这个？  判断就专指他是否为一个特定的值？
		v_equ(out, curr, size);		// 反正有三种情况
	else {
...
/* ============================================ */
// output 为 Q0 , 就是小数部分被舍弃，将整数作为最终结果
...
/* ============================================== */
// window_Q(par->fs_mag, w_fs_inv, par->fs_mag, NUM_HARM, 14);  在syn里这么使用
void window_Q(int16_t input[], int16_t win_coeff[], int16_t output[],	// 给出 Q 值，可以控制数据精度，调整数据范围

...
#include "sc1200.h"		/*  sc1200 只有一个 h文件  引用 里面定义的参数*/
#include "cprv.h"		/*	cprv 仅一个 h 文件 , 当前脚本的函数声明在cprv，包括当前使用道德结构体*/
#include "global.h"		/*	全局变量常量的声明与使用	*/
#include "mat_lib.h"
#include "macro.h"		/*		系统判断啥的？ 没看太懂   */
#include "mathhalf.h"
#include "constant.h"		/*		定义了很多常数		*/
#include "math_lib.h"
#include "dsp_sub.h"
#include "coeff.h"			/*		定义系数常量		*/
#include "melp_sub.h"
...
#define SILENCE_DB_Q11			6144	/* Originally 30.  subEnergy is */
				     /* changed to 1/10 of its floating point */
					  /* counterpart.  30/10 * (1 << 11). */   // 算出来也是 3 乘 2的11次方
#define TWO_Q11					4096	/* 2 * (1 << 11)  2 乘 2的11次方 */ 
#define THREE_Q11				6144	/* 3 * (1 << 11)  3 乘 2的11次方*/
#define FIVE_Q11				10240	/* 5 * (1 << 11)  5 乘 2的11次方*/
#define M_TEN_Q11				-20480   // 负的？类上
#define X04_Q15					13107	/* 0.4 * (1 << 15) */  // 小数？！ 类上形式 ，只是不同
#define X045_Q15				14746	/* 0.45 * (1 << 15) */
#define X05_Q12					2048	/* 0.5 * (1 << 12) */
#define X055_Q15				18022	/* 0.55 * (1 << 15) */
#define X06_Q15					19661	/* 0.6 * (1 << 15) */
#define X065_Q15				21299	/* 0.65 * (1 << 15) */   // X表示小数， 第一个数字为 第一位！
#define X14_Q11					2867	/* 1.4 * (1 << 15) */
#define X15_Q11					3072	/* 1.5 * (1 << 11) */
#define X16_Q11					3277	/* 1.6 * (1 << 11) */
#define LOG2_Q11				617	/* log10(2.0) * (1 << 11) */
#define ONE_OV_SQRT2_Q15		23170	/* 1/sqrt(2) * (1 << 15) */   // 是 1 除 根号2 ， 不是给1开根号！
...
// 计算音频信号中的零交叉率，分析speech中的正负交替来估计信号的频率特性或其他相关信息
static int16_t bandEn(int16_t autocorr[], int16_t band); // autocorr存储自相关系数数据
// 计算自相关系数在指定频带内的能量，以评估信号在该频带内的强度或其他相关特性
static void frac_cor(int16_t inbuf[], int16_t pitch, int16_t * cor);
//  int16_t* 类型的指针变量 cor，用于存储计算出的自相关系数
...
** Arguments:
**
**	int16_t inbuf[]		speech buffer (Q0)   信号缓冲区，以Q0格式表示。Q0表示该信号是无定点小数表示，没有进行任何位移或缩放
**	classParam *classStat	classification structure   分类结构体指针，用于存储和更新分类相关的状态和结果
**  int16_t autocorr[]	autocorrelation coefficients (normalized, Q value
**                          irrelevant)    已经归一化的自相关系数数组 ， Q 值不重要， 不影响函数使用
**
...
{
	register int16_t i, j;    // Q都是表示数字的精度
...
** Arguments:
**
**	int16_t speech[]		Input speech buffer (Q0)  // speech 语音信号多是 整数
**
** Return value:
**	int16_t				Zero Crossing Rate (Q15)   小数 转化为整数输出 且非数组
**
*****************************************************************************/
static int16_t zeroCrosCount(int16_t speech[])
{
...
** Arguments:
**
**	int16_t inbuf[]	---- input data buffer (Q0)    输入数据的缓存区
**	int16_t pitch		---- the input pitch center (Q0)  给的 pitch ，但是整数！
**	int16_t *cor		---- output cor coeff (Q15)  指针返回相关系数
**
** Return value:	None
**
*****************************************************************************/

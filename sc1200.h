...
#define FRAME				180	/* speech frame size */		// MELP_ANA (static) 用到了
#define NF					3	/* number of frames in one block */
#define BLOCK				(NF*FRAME)	// 所以 一个 BLOCK 是 540 
...
#define PITCHMAX_Q7			20480	/* PITCHMAX * (1 << 7) */	// 表示方法
...
struct melp_param {		/* MELP parameters */
	int16_t pitch;	/* Q7 */					// 1	chn 5	ana  y
	int16_t lsf[LPC_ORD];	/* Q15 */			// 10	chn 1	ana  1 lpc
	int16_t gain[NUM_GAINFR];	/* Q8 */		// 2	chn 3	ana  y
	int16_t jitter;	/* Q15 */					// 1	chn 6	ana  y
	int16_t bpvc[NUM_BANDS];	/* Q14 */		// 5	chn 7	ana  y
	BOOLEAN uv_flag;							// 1	chn 4	ana  y
	int16_t fs_mag[NUM_HARM];	/* Q13 */		// 10	chn 2	ana  n	量化部分出现 analysis
};
...
void analysis(int16_t sp_in[], struct melp_param *par);   //  提取参数， 增强后的speech，提取出模型参数（结构体形式）
void getpar(int16_t sp_in[], struct melp_param *par);		// 自己写的
void sc_ana(struct melp_param *par);  // ？？？

void synthesis(struct melp_param *par, int16_t sp_out[]);  //  量化后
void rmquan(struct melp_param *par, int16_t sp_out[], struct melp_param *par2);		// 没量化直接给参数
